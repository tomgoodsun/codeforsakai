...
/**
 * @file
 * Contains functions to alter Drupal's markup for the cfs theme.
 *
 * IMPORTANT WARNING: DO NOT MODIFY THIS FILE.
 *
 * The base cfs theme is designed to be easily extended by its sub-themes. You
 * shouldn't modify this or any of the CSS or PHP files in the root cfs/ folder.
 * See the online documentation for more information:
 *   https://drupal.org/documentation/theme/cfs
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('cfs_rebuild_registry') && !defined('MAINTENANCE_MODE')) {
  // Rebuild .info data.
 ...
 */
function cfs_theme(&$existing, $type, $theme, $path) {
  include_once './' . drupal_get_path('theme', 'cfs') . '/assets/template.theme-registry.inc';
  return _cfs_theme($existing, $type, $theme, $path);
}
 ...
 */
function cfs_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  $output = '';

  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('cfs_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('cfs_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = filter_xss_admin(theme_get_setting('cfs_breadcrumb_separator'));
      $trailing_separator = $title = '';
      if (theme_get_setting('cfs_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('cfs_breadcrumb_trailing')) {
 ...
 *   also be "maintenance_page" since cfs_preprocess_maintenance_page() calls
 *   this function to have consistent variables.
 */
function cfs_preprocess_html(&$variables, $hook) {
  // Add variables and paths needed for HTML5 and responsive support.
  $variables['base_path'] = base_path();
  $variables['path_to_cfs'] = drupal_get_path('theme', 'cfs');
  // Get settings for HTML5 and responsive support. array_filter() removes
  // items from the array that have been disabled.
  $html5_respond_meta = array_filter((array) theme_get_setting('cfs_html5_respond_meta'));
  ...
  // If the user is silly and enables cfs as the theme, add some styles.
  if ($GLOBALS['theme'] == 'cfs') {
    include_once './' . $variables['path_to_cfs'] . '/assets/template.cfs.inc';
    _cfs_preprocess_html($variables, $hook);
  }
  ...
  $variables['skip_link_anchor'] = check_plain(theme_get_setting('cfs_skip_link_anchor'));
  $variables['skip_link_text']   = check_plain(theme_get_setting('cfs_skip_link_text'));
  ...
  if (theme_get_setting('cfs_wireframes')) {
 ...
 */
function cfs_process_html(&$variables, $hook) {
 ...
 */
function cfs_process_html_tag(&$variables) {
 ...
 */
function cfs_html_head_alter(&$head) {
 ...
 */
function cfs_preprocess_page(&$variables, $hook) {
 ...
 */
function cfs_preprocess_maintenance_page(&$variables, $hook) {
  cfs_preprocess_html($variables, $hook);
  // There's nothing maintenance-related in cfs_preprocess_page(). Yet.
  //cfs_preprocess_page($variables, $hook);
}
 ...
 *   The name of the template being rendered ("maintenance_page" in this case.)
 */
function cfs_process_maintenance_page(&$variables, $hook) {
  cfs_process_html($variables, $hook);
 ...
 */
function cfs_preprocess_node(&$variables, $hook) {
 ...
 */
function cfs_preprocess_comment(&$variables, $hook) {
 ...
 */
function cfs_preprocess_region(&$variables, $hook) {
  // Sidebar regions get some extra classes and a common template suggestion.
  if (strpos($variables['region'], 'sidebar_') === 0) {
    $variables['classes_array'][] = 'column';
    $variables['classes_array'][] = 'sidebar';
    // Allow a region-specific template to override cfs's region--sidebar.
  ...
  elseif ($variables['region'] == 'content') {
    // Allow a region-specific template to override cfs's region--no-wrapper.
 ...
 */
function cfs_preprocess_block(&$variables, $hook) {
  ...
  // The last_in_region property is set in cfs_page_alter().
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function cfs_process_block(&$variables, $hook) {
 ...
 */
function cfs_form_node_form_alter(&$form, &$form_state, $form_id) {
 ...
 * @ingroup themeable
 */
function cfs_menu_local_tasks(&$variables) {
  $output = '';
 ...
 * @ingroup themeable
 */
function cfs_menu_local_task($variables) {
  $type = $class = FALSE;

  $link = $variables['element']['#link'];
  $link_text = $link['title'];

  // Check for tab type set in cfs_menu_local_tasks().
 ...
 */
function cfs_preprocess_menu_link(&$variables, $hook) {
 ...
 */
function cfs_status_messages($variables) {
 ...
 */
function cfs_panels_default_style_render_region($variables) {
